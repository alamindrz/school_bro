...
from apps.corecode.models import Subject
    ...
    def clean(self):
        ...
        if teacher and subject:
            # Check if qualification already exists (excluding current instance)
                teacher=teacher,
                subject=subject
            )
            if self.instance.pk:
...
class TimetableSlotUpdateForm(forms.Form):
    ...
    def clean(self):
        ...
        # If not free period and teacher selected, subject is required
...
class TimetableForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'name': forms.TextInput(attrs={'class': 'w-full px-3 py-2 border rounded-lg', 'placeholder': 'e.g., SS1A First Term'}),
...
class BulkQualificationForm(forms.Form):
    ...
    is_primary = forms.BooleanField(required=False, initial=False, label="Set as primary subject")
